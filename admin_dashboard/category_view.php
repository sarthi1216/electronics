...
$result = $conn->query("SELECT * FROM electronics_categories WHERE admin_id=$adminID");
...
<body>
   ...
   <section>
   <header>
            <h2>View Electronics Categories</h2>
    ...
    <main>
        ...
        <table class="table table-striped">
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>{$row['id']}</td>";
                echo "<td>{$row['name']}</td>";
                echo "<td><img src='{$row['image']}' alt='{$row['name']}' style='max-width: 50px; max-height: 50px;'></td>";
                echo "<td>{$row['display_order']}</td>";
                echo "<td>{$row['state']}</td>";
                echo "<td class='action-buttons'>
                        <a href='update_category.php?id={$row['id']}'><i class='fas fa-edit'></i> Update</a>
                        <a href='category_view.php?delete={$row['id']}' class='delete'><i class='fas fa-trash-alt'></i> Delete</a>
                        <a href='product_view.php?category_id={$row['id']}'><i class='fas fa-glasses'></i> View Products</a>
                      </td>";
                echo "</tr>";
            }
            ?>
